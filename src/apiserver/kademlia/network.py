...
class Server:
    ...
    def __init__(self, ksize=2, alpha=3, node_id=None, storage=None,check_interval=30, heartbeat_timeout=5):
        """
        ...
        self.check_interval = check_interval  # Heartbeat interval in seconds
        self.heartbeat_timeout = heartbeat_timeout  # Timeout to consider a node as failed

        # Failed nodes set
        self.failed_nodes = set()

        # Start the health check task
        self.health_check_loop = None
    ...
    async def _health_check(self):
        ...
        """
        while True:
            log.debug("Starting heartbeat check...")
            tasks = []
            
            for key in self.storage.data.keys():
                actValue= self.storage.data[key]
                tasks.append(self.set(key,actValue))
            

            # Wait for all keys set to complete
